...
const MODULE_CONFIG = [
  ...
  },
  {
    ...
    features: {
      'pt-BR': ['DASHBOARD CENTRALIZADO', 'GESTÃO DE MÚLTIPLOS PROJETOS'],
      en: ['CENTRALIZED DASHBOARD', 'MULTI-PROJECT MANAGEMENT'],
      it: ['DASHBOARD CENTRALIZZATA', 'GESTIONE MULTI-PROGETTO'],
    },
...
export function ModuleSelector({
  ...
  return (
    <div className="min-h-screen bg-midnight text-midnight-text flex flex-col items-center justify-center p-8 transition-colors overflow-hidden relative">
      ...
      {/* ── Cards Container ─────────────────────────────────────────────────── */}
      <div
        className={`grid gap-8 w-full relative z-10 ${visible.length === 1
            ? 'max-w-md mx-auto'
            : visible.length === 2
              ? 'md:grid-cols-2 max-w-4xl mx-auto'
              : 'lg:grid-cols-4 max-w-[1400px] mx-auto'
          }`}
      >
        {visible.map((mod, i) => {
          const Icon = mod.icon;
