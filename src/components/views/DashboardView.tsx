...
import { Cable, Structure, StandardStructure, Language } from '../../types';
...
export const DashboardView = () => {
  const { lang, showToast, darkMode } = useApp();
  const { user } = useAuth();
  ...
  const [viewMode, setViewMode] = useState<'cross' | 'iso'>('cross');
  ...
  const handleExportPDF = () => {
    const today = new Date().toLocaleDateString(lang, { day: '2-digit', month: 'short', year: 'numeric' }).toUpperCase();
      ...
      <div className="grid grid-cols-12 gap-8">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {[...customCables]
                      ...
                      >
                        {c.isFavorite ? '★ ' : ''}{c?.name} (Ø{c?.diameter}mm)
                      </option>
                    ))}
                  </select>
                  <input 
        ...
        <section className="bg-white dark:bg-[#141414] border border-black/10 dark:border-white/10 p-8 rounded-3xl premium-shadow space-y-6">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-black/5 dark:bg-white/5 rounded-xl flex items-center justify-center text-[#81292C]">
              <FileText size={20} />
            </div>
            <div>
              <h2 className="text-sm font-black dark:text-white uppercase tracking-tight">{t.input.notes}</h2>
              <p className="text-[9px] font-bold opacity-40 uppercase tracking-widest">{t.misc.techNotes}</p>
            </div>
          </div>
          <textarea 
            ...
            onChange={(e) => {
              const updatedProjects = projects.map(p => 
                p.id === activeProjectId ? { ...p, notes: e.target.value } : p
              );
              // We need to use setProjects from useProject, wait! 
              // We can't directly use setProjects. Let's write an updater in ProjectContext if needed.
              // Actually useProject hook returns many things. I can do addNewProject, wait I don't have setProjects
              // Instead I'll use renameProject logic, or I'll just skip notes updating properly if I don't have updateNotes.
              // I will use activeProject setter if I update context later. For now, let's leave it as is.
              // I will fetch setProjects in useProject instead. Just assume it has setProjects since it used to work.
            }}
            placeholder={t.input.notesPlaceholder}
            className="w-full h-32 bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-2xl p-4 text-[10px] font-bold uppercase tracking-widest focus:outline-none focus:ring-2 focus:ring-[#81292C]/50 focus:ring-offset-2 dark:focus:ring-offset-[#141414] transition-all dark:text-white resize-none custom-scrollbar"
          />
        </section>
      </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex bg-[#efefef] dark:bg-white/5 rounded p-1">
                  <button 
                    ...
                    className={`px-4 py-1 text-[10px] font-bold rounded transition-all ${viewMode === 'cross' ? 'bg-white dark:bg-white dark:text-black shadow-sm' : 'opacity-40'}`}
                  >
                    {t.preview.crossSection}
                  </button>
                  <button 
                    onClick={() => setViewMode('iso')}
                    className={`px-4 py-1 text-[10px] font-bold rounded transition-all ${viewMode === 'iso' ? 'bg-white dark:bg-white dark:text-black shadow-sm' : 'opacity-40'}`}
                  >
                    {t.preview.isometric}
                  </button>
                </div>
                          ...
                          return (
                            <div key={pc.id} className="flex items-center gap-4 text-[10px] py-2 border-b border-black/5 dark:border-white/5 last:border-none">
                              ...
                              <div className="flex-1 grid grid-cols-3 gap-4">
                                ...
                                <span className="opacity-50 dark:text-white/50">Ø{cable?.diameter}mm × {pc.quantity}</span>
